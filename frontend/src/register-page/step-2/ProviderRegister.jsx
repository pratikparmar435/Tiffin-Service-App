import { MapPin } from "lucide-react";
import { useState } from "react";

const ProviderRegister = () => {
  const [providerFormData, setProviderFormData] = useState({
    serviceName: "",
    ownerName: "",
    number: "",
    email: "",
    location: "",
    password: "",
  });

  const totalFields = Object.keys(providerFormData).length;

  const filledFields = Object.values(providerFormData).filter(
    (value) => value.trim() !== "",
  ).length;

  const stepBase = 50;
  const stepRange = 50;

  const progressPercent =
    stepBase + Math.round((filledFields / totalFields) * stepRange);

  function handleInputs(event) {
    let fieldName = event.target.name;
    let newValue = event.target.value;
    setProviderFormData((currData) => {
      currData[fieldName] = newValue;
      return { ...currData };
    });
  }

  function handleSubmit(event) {
    event.preventDefault();
    console.log(providerFormData);
    setProviderFormData({
      serviceName: "",
      ownerName: "",
      number: "",
      email: "",
      location: "",
      password: "",
    });
  }
  return (
    <div className="flex flex-col justify-center items-center">
      {/* Main  */}
      <div className="w-135 rounded-2xl my-8 bg-white flex flex-col p-10 shadow-sm">
        {/* Heading  */}
        <div>
          <h1 className="font-bold text-2xl ">Create Your Account</h1>
          <span className="text-sm tracking-tight text-[#ae876a]">
            Tell us about your tiffin service
          </span>
        </div>
        {/* Status bar  */}
        <div className="rounded-2xl mt-6 flex flex-col">
          <div className="flex items-center justify-between">
            <h5 className=" tracking-tighter">Registration Progress</h5>
            <span className="text-sm tracking-tighter text-[#ae876a] font-light">
              {progressPercent}% Complete
            </span>
          </div>
          <div className="status-bar my-2 w-full h-2 bg-[#e8d9ce] rounded-xl overflow-hidden">
            <div
              className="h-full bg-[#f47b25] transition-all duration-300"
              style={{ width: `${progressPercent}%` }}
            />
          </div>
          <span className="text-sm tracking-tighter text-[#ae876a] mb-6">
            Finalizing your provider profile
          </span>
        </div>
        <form action="" onSubmit={handleSubmit} className="flex flex-col">
          <div className="flex items-center gap-3">
            <div className="flex-1">
              <label
                htmlFor="serviceName"
                className="text-sm tracking-tighter font-semibold mb-1"
              >
                Service Name
              </label>
              <input
                type="text"
                placeholder="e.g. Mama's Kitchen"
                id="serviceName"
                name="serviceName"
                onChange={handleInputs}
                value={providerFormData.serviceName}
                className="bg-white w-full outline-none p-3 border border-[#F2D4BE] rounded-lg text-[#e39964]"
              />
            </div>
            <div className="flex-1">
              <label
                htmlFor="ownerName"
                className="text-sm tracking-tighter font-semibold mb-1"
              >
                Owner Name
              </label>
              <input
                type="text"
                placeholder="Enter you full name"
                id="ownerName"
                name="ownerName"
                onChange={handleInputs}
                value={providerFormData.ownerName}
                className="bg-white w-full outline-none p-3 border border-[#F2D4BE] rounded-lg text-[#e39964]"
              />
            </div>
          </div>
          <div className="mt-4 flex gap-3">
            <div className="flex-1">
              <label
                htmlFor="number"
                className="text-sm tracking-tighter font-semibold mb-1"
              >
                Phone Number
              </label>
              <input
                type="number"
                placeholder="+1(555)000-0000"
                id="number"
                name="number"
                onChange={handleInputs}
                value={providerFormData.number}
                className="bg-white w-full outline-none p-3 border border-[#F2D4BE] rounded-lg text-[#d4a584]"
              />
            </div>
            <div className="flex-1">
              <label
                htmlFor="email"
                className="text-sm tracking-tighter font-semibold mb-1"
              >
                Business Email
              </label>
              <input
                type="email"
                placeholder="kashyap@example.com"
                id="email"
                value={providerFormData.email}
                name="email"
                onChange={handleInputs}
                className="bg-white w-full outline-none p-3 border border-[#F2D4BE] rounded-lg text-[#d4a584]"
              />
            </div>
          </div>
          <label
            htmlFor="location"
            className="text-sm tracking-tighter font-semibold mb-1 mt-4"
          >
            City / Area
          </label>
          <input
            type="text"
            placeholder={`e.g. Sola Bhagwat`}
            id="location"
            name="location"
            value={providerFormData.location}
            onChange={handleInputs}
            className="bg-white w-full outline-none p-3 border border-[#F2D4BE] rounded-lg text-[#d4a584]"
          />

          <label
            htmlFor="password"
            className="text-sm tracking-tighter font-semibold mb-1 mt-4"
          >
            Password
          </label>
          <input
            type="password"
            placeholder="Create password"
            id="password"
            name="password"
            onChange={handleInputs}
            value={providerFormData.password}
            className="bg-white w-full outline-none p-3 border border-[#F2D4BE] rounded-lg text-[#d4a584]"
          />
          <span className="text-sm tracking-tighter mt-8 mb-3 text-center">
            I agree to the{" "}
            <a href="#" className="text-[#317f35] underline">
              Terms & Conditions
            </a>{" "}
            and Private Policy.
          </span>
          <button className="bg-[#f47b25] p-3 cursor-pointer text-white font-semibold rounded-lg">
            Create Account
          </button>
          <p className="text-center mt-6 text-[#ae876a] text-sm">
            Already have an account?{" "}
            <a
              href="#"
              className="text-sm text-[#f47b25] tracking-tighter font-semibold"
            >
              Login here
            </a>
          </p>
        </form>
      </div>
    </div>
  );
};

export default ProviderRegister;
